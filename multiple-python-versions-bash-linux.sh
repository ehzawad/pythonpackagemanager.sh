...
declare -Ag _PYTHON_PATHS=()
declare -Ag _PYTHON_INFO=()
declare -g _VENV_PYTHON_VERSION_CACHE=""
declare -g _LAST_VIRTUAL_ENV=""
declare -g _PYTHONS_SCANNED=0
declare -g _PYTHON_OVERRIDE=""
...
_scan_all_pythons() {
    ...
    # All possible Python locations on macOS/Linux
    local search_paths=(
    ...
    for py in "${python_executables[@]}"; do
        ...
        if [[ "$(basename "$py")" =~ ^python([0-9]+\.?[0-9]*)$ ]]; then
        ...
        if fullver=$("$py" --version 2>&1); then
            if [[ "$fullver" =~ Python\ ([0-9]+)\.([0-9]+)\.?[0-9]* ]]; then
                local extracted_version="${BASH_REMATCH[1]}.${BASH_REMATCH[2]}"
        ...
        local realpath="$py"
        if [[ -L "$py" ]]; then
            realpath=$(readlink -f "$py" 2>/dev/null || readlink "$py" 2>/dev/null || echo "$py")
        fi
    ...
    # Sort versions numerically (bash way)
    if [[ ${#_PYTHON_VERSIONS[@]} -gt 0 ]]; then
        IFS=$'\n' _PYTHON_VERSIONS=($(printf '%s\n' "${_PYTHON_VERSIONS[@]}" | sort -V | sort -u))
        unset IFS
    fi
    _PYTHONS_SCANNED=1
}
...
python() {
    ...
    echo ""
    echo "💡 Options:"
    echo "   1. Create venv: python${sorted_versions[0]} -m venv [venv-projname] && source [venv-projname]/bin/activate"
    echo "   2. Set temporary default: setpy ${sorted_versions[0]}"
